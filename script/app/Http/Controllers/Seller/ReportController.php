...
class ReportController extends Controller
{
    public function index(Request $request)
    {
    	...
    	$start=$request->start ?? '';
    	$end=$request->end ?? '';
        $posts = Term::where('type', 'product')->with('preview')->withCount('order')->where('status', 1)->where('user_id', $user_id)->latest()->paginate(30);
    	return view('seller.report.index',compact('orders','start','end','total','completed','canceled','proccess','amounts','amount_cancel','amount_proccess','amount_completed','request', 'posts'));
    }
}
