                ...
                <div class="card-header">
                    ...
                    <form class="card-header-form">
                        <div class="d-flex">
                            ...
                            <button class="btn btn-primary btn-icon" type="submit"><i class="fas fa-search"></i>
                            </button>
                        </div>
                    </form>
                </div>
                        ...
                        <table class="table table-striped table-md table-hover">
                            ...
                            @foreach($orders as $key => $row)
                                <tr>
                                    ...
                                    <td> @if(empty($row->user_id)) {{ __('Guest Order') }} @else <a
                                            href="{{ route('seller.customer.show',$row->user_id) }}">{{ $row->customer->name ?? '' }}</a>  @endif
                                    </td>
                        ...
                        <table class="table">
                            ...
                            @foreach($posts as $row)
                                <tr id="row{{  $row->id }}">
                                    ...
                                    <td><img src="{{ asset($row->preview->media->url ?? 'uploads/default.png') }}" height="50"
                                             alt=""></td>
                                    <td>{{ $row->title }} (#{{ $row->id }})
                                        <div>
                                            <a href="{{ route('seller.product.edit',$row->id) }}">{{ __('Edit') }}</a> |
                                            <a href="{{ $url.'/product/'.$row->slug.'/'.$row->id }}"
                                               target="_blank">{{ __('Show') }}</a> |
                                            <a href="{{ route('seller.statistic.show',$row->id) }}">{{ __('Statistic') }}</a>
                                        </div>
                                    </td>
