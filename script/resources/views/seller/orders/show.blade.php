					...
					<li class="list-group-item">
						...
						<li class="list-group-item">
							<div class="row align-items-center">
								...
								<div class="col-6">
                                    {{ $order_content->disctrict ?? '' }}, {{ $order_content->city ?? '' }}, {{ $order_content->province ?? '' }}
								</div>
								<div class="col-3 text-right">
									{{ __('Shipping Fee') }}
								</div>
		...
		<div class="col-12 col-lg-4">
			<div class="card-grouping">
                <div class="card">
                    ...
                    <div class="card-body">
                        <p>{{ __('Input AWB') }}
                        <form action="{{ route('seller.awb.store', $info->id) }}" method="post" >
                            @csrf
                            <input name="awb" value="{{ $order_shipping  ?? ''}}" type="text" class="form-control" >
                            <br>
				...
				<div class="card">
					...
					<div class="card-body">
						...
						<p class="mb-0">{{ __('Shipping Method') }}: JNE - {{ $order_content->service ?? '' }}</p>
						<p class="mb-0">{{ __('Destination') }}: {{ $order_content->disctrict ?? '' }}</p>
						<p class="mb-0">{{ __('Estimation') }}: {{ $order_content->estimation ?? '' }} {{ __('days') }}</p>
					</div>
				</div>
