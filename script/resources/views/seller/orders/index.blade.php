...
@section('content')
            ...
            <div class="col-12">
                    ...
                    <div class="card-header">
                        <div class="col-sm-10">
                            <ul class="nav nav-pills">
                                <li class="nav-item">
                                    <a class="nav-link @if(url()->current() == route('seller.orders.status','all')) active @endif"
                                       href="{{ route('seller.orders.status','all') }}">{{ __('All') }}</a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link @if($type == 'pending') active @endif"
                                       href="{{ route('seller.orders.status','pending') }}">{{ __('Awaiting processing') }}
                                        <span class="badge badge-secondary">{{ $pendings }}</span></a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link  @if($type == 'processing') active @endif"
                                       href="{{ route('seller.orders.status','processing') }}">{{ __('Processing') }}
                                        <span class="badge badge-secondary">{{ $processing }}</span></a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link @if($type == 'pickup') active @endif"
                                       href="{{ route('seller.orders.status','pickup') }}">{{ __('Ready for pickup') }}
                                        <span class="badge badge-secondary">{{ $pickup }}</span></a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link @if($type == 'completed') active @endif"
                                       href="{{ route('seller.orders.status','completed') }}">{{ __('Completed') }}
                                        <span class="badge badge-secondary">{{ $completed }}</span></a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link @if($type == 'canceled') active @endif"
                                       href="{{ route('seller.orders.status','canceled') }}">{{ __('Cancelled') }} <span
                                            class="badge badge-secondary">{{ $canceled }}</span></a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link @if($type == 'archived') active @endif"
                                       href="{{ route('seller.orders.status','archived') }}">{{ __('Archived') }} <span
                                            class="badge badge-secondary">{{ $archived }}</span></a>
                                </li>
                            </ul>
                        </div>
                        <div class="col-sm-2">
                ...
                <div class="card">
                    <div class="card-header">
                        ...
                        <button class="btn btn-sm btn-primary  ml-1" type="button" data-toggle="modal"
                                data-target="#searchmodal">
                            <i class="fe fe-sliders mr-1"></i> {{ __('Filter') }} <span
                                class="badge badge-primary ml-1 d-none">0</span>
                        </button>
                                ...
                                @if(count($orders) > 0)
                                    <div class="input-group mb-1">
                                        <select class="form-control selectric" name="method">
                                            ...
                                            @if($type== 'canceled')
                                                <option value="delete"
                                                        class="text-danger">{{ __('Delete Permanently') }}</option>
                            ...
                            <div class="table-responsive">
                                <table class="table table-hover table-nowrap card-table text-center">
                                    ...
                                    <tr>
                                        ...
                                        <th class="text-center">{{ __('Chats') }}</th>
                                    </tr>
                                    </thead>
                                    ...
                                    @foreach($orders as $key => $row)
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <a href="{{ route('seller.order.show',$row->id) }}">{{ $row->created_at->format('d-F-Y') }}</a>
                                            </td>
                                            <td>
                                                @if($row->customer_id !== null)
                                                    <a href="{{ route('seller.customer.show',$row->customer_id) }}">
                                                        {{ $row->customer->name ?? ''}}</a>
                                                @else {{ __('Guest User') }}
                                                @endif</td>
                                            ...
                                            <td><a href="{{ route('seller.invoice',$row->id) }}"
                                                   class="btn btn-primary btn-sm"><i
                                                        class="fas fa-file-invoice"></i></a></td>
                                            <td>
                                                @if($row->guest_id == NULL)
                                                    <a href="#" data-toggle="modal"
                                                       data-target="#chats-{{$row->customer_id}}"
                                                       class="btn btn-success btn-sm"><i class="fas fa-envelope"></i>
                                                    </a>
                                                @else
    ...
    <!-- Modal -->
    @foreach($orders as $key => $row)
        <div class="modal fade" id="chats-{{$row->customer_id}}" tabindex="-1"
             role="dialog"
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title"
                            id="exampleModalLongTitle"> {{ $row->customer->name ?? 'Guest Customer'}}</h5>
                        ...
                        <div class="container ">
                            <div class="row ">
                                <div class="col-12 px-0">
                                    ...
                                    @foreach($chat_seller as $ami)
                                        @if($row->customer_id == $ami->customer_id)
                                            @if($ami->role == 'customer')
                                                <!-- Sender Message-->
                                    ...
                                    <!-- Typing area -->
                                    <form id="chatds" action="{{url('seller/orders/chat') }}"
                                        ...
                                        <div class="input-group">
                                            ...
                                            <div class="input-group-append">
                                                <button form="chatds" id="button-addon2" type="submit"
