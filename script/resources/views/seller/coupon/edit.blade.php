        ...
        <form class="basicform" action="{{ route('seller.coupon.update',$info->id) }}" method="post">
          ...
          @method('PUT')
            <div class="form-group row mb-4">
                <label class="col-form-label text-md-right col-12 col-md-3 col-lg-3" >{{ __('Product') }}</label>
                <div class="col-sm-12 col-md-7">
                    <select name="p_id" class="form-control" id="">
                        @foreach($data as $row)
                            <option class="form-control" value="{{ $row->id }}">{{ $row->title }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
